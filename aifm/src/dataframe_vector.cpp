...
void GenericDataFrameVector::cleanup() {
  if (!moved_) {
    {
      // Scope to make sure writer lock is released before duo.reset()
      auto writer_lock = duo->lock_.get_writer_lock();      
      std::vector<rt::Thread> threads;
      for (uint32_t tid = 0; tid < helpers::kNumCPUs; tid++) {
        threads.emplace_back([&, tid]() {
          auto num_tasks_per_threads =
              (duo->chunk_ptrs_.size() == 0)
                  ? 0
                  : (duo->chunk_ptrs_.size() - 1) / helpers::kNumCPUs + 1;
          auto left = num_tasks_per_threads * tid;
          auto right = std::min(left + num_tasks_per_threads, duo->chunk_ptrs_.size());
          for (uint64_t i = left; i < right; i++) {
            duo->chunk_ptrs_[i].free();
          }
        });
      }
...
void GenericDataFrameVector::expand(uint64_t num) {
  auto old_chunk_ptrs_size = duo->chunk_ptrs_.size();
  uint64_t new_capacity = (old_chunk_ptrs_size + num) * chunk_num_entries_;
  //reserve_remote(new_capacity);
  auto writer_lock = duo->lock_.get_writer_lock();
  duo->chunk_ptrs_.resize(old_chunk_ptrs_size + num);
  ...
  for (uint32_t tid = 0; tid < helpers::kNumCPUs; tid++) {
    threads.emplace_back([&, tid]() {
      ...
      for (uint64_t i = left; i < right; i++) {
        ...
        auto &new_chunk_ptr = duo->chunk_ptrs_[obj_id];
