...
//#define PRINT_OUTPUT
constexpr bool is_constant_file_name = false;
//std::string file_name_prefix("/mnt/ssd/data/chunks/yellow_tripdata_2016-01-");
std::string file_name_prefix("/mnt/ssd/data/chunks1000/yellow_tripdata_2016-01-");
std::string file_name_suffix(".csv");
std::string file_name_constant("/mnt/ssd/data/yellow_tripdata_2016-01.csv");
//std::string file_name_constant("/mnt/ssd/data/all.csv");
constexpr unsigned int num_files = is_constant_file_name ? 1 : 50;
constexpr unsigned int starting_num_of_file = 0;
constexpr unsigned int ending_num = starting_num_of_file + num_files;
unsigned int num_of_files_each_heartbeat = (num_files / 100) >= 1 ? (num_files / 100) : 1;

using namespace hmdf;
using namespace far_memory;

constexpr uint64_t kCacheMBs = 8000;
constexpr uint64_t kCacheSize           = kCacheMBs << 20;
constexpr uint64_t kFarMemSize          = (36ULL << 30);  // 1 GB. Not relevant here.
...
StdDataFrame<Index_t> load_data(FarMemManager* manager, int index)
{
    std::string file_name;
    if (is_constant_file_name) {
        file_name = file_name_constant;
    } else {
        file_name = file_name_prefix + std::to_string(index) + file_name_suffix;
    }
    std::string ratecode_spelling = "RateCodeID";
    if (is_constant_file_name) {
        ratecode_spelling = "RatecodeID";
    }
    return read_csv<-1, int, SimpleTime, SimpleTime, int, double, double, double, int, char, double,
        ...
        manager, file_name, "VendorID", "tpep_pickup_datetime", "tpep_dropoff_datetime",
        "passenger_count", "trip_distance", "pickup_longitude", "pickup_latitude", ratecode_spelling.c_str(),
...
void do_work(FarMemManager* manager)
{
    ...
    //std::cout << sizeof(StdDataFrame<Index_t>) << std::endl;
    //std::cout << sizeof(GenericDataFrameVector) << std::endl;
    //std::cout << sizeof(DataFrameVector<double>) << std::endl;
    std::cout << "start loading" << std::endl;
    time_before_load = std::chrono::steady_clock::now();
    std::vector<StdDataFrame<Index_t>> dfs;
    #ifndef PRINT_OUTPUT
        std::cout.setstate(std::ios::failbit);
    #endif // PRINT_OUTPUT
    dfs.reserve(num_files);
    for (int i = starting_num_of_file; i < ending_num; ++i) {
        dfs.emplace_back(load_data(manager, i));
        std::cout << "loaded one" << std::endl;
        if (false && (i + 1) % num_of_files_each_heartbeat == 0) {
            #ifndef PRINT_OUTPUT
                std::cout.clear();
            #endif // PRINT_OUTPUT
            std::cout << "load heartbeat" << std::endl;
            #ifndef PRINT_OUTPUT
                std::cout.setstate(std::ios::failbit);
            #endif // PRINT_OUTPUT
        }
    ...
    std::cout << "time taken to load: " << std::chrono::duration_cast<std::chrono::microseconds>(times[0] - time_before_load).count() << " us" << std::endl;
        ...
        std::cout.setstate(std::ios::failbit);
    #endif // PRINT_OUTPUT
    num_of_files_each_heartbeat = 1;
    for (int i = 0; i < num_files; ++i) {
        print_number_vendor_ids_and_unique(manager, dfs[i]);
    times[1] = std::chrono::steady_clock::now();
        print_passage_counts_by_vendor_id(manager, dfs[i], 1);
    times[2] = std::chrono::steady_clock::now();
        print_passage_counts_by_vendor_id(manager, dfs[i], 2);
    times[3] = std::chrono::steady_clock::now();
        calculate_trip_duration(manager, dfs[i]);
    times[4] = std::chrono::steady_clock::now();
        calculate_distribution_store_and_fwd_flag(manager, dfs[i]);
    times[5] = std::chrono::steady_clock::now();
        calculate_haversine_distance_column(manager, dfs[i]);
    times[6] = std::chrono::steady_clock::now();
        analyze_trip_timestamp(manager, dfs[i]);
    times[7] = std::chrono::steady_clock::now();
        analyze_trip_durations_of_timestamps<char>(manager, dfs[i], "pickup_day");
    times[8] = std::chrono::steady_clock::now();
        analyze_trip_durations_of_timestamps<char>(manager, dfs[i], "pickup_month");
        std::cout << "finished one" << std::endl;
        if ((i + 1) % num_of_files_each_heartbeat == 0) {
            #ifndef PRINT_OUTPUT
                std::cout.clear();
            #endif // PRINT_OUTPUT
            std::cout << "process heartbeat" << std::endl;
            #ifndef PRINT_OUTPUT
                std::cout.setstate(std::ios::failbit);
            #endif // PRINT_OUTPUT
        }
