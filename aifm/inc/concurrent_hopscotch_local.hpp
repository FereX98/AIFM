...
class GenericConcurrentHopscotchLocal {
private:
  struct BucketEntry {
...
#pragma pack(pop)
  ...
  template <typename K, typename V> friend class ConcurrentHopscotchLocal;

  GenericConcurrentHopscotchLocal(uint8_t ds_id, uint32_t local_num_entries_shift,
                             uint32_t remote_num_entries_shift,
                             uint64_t remote_data_size);
  NOT_COPYABLE(GenericConcurrentHopscotchLocal);
  NOT_MOVEABLE(GenericConcurrentHopscotchLocal);
...
class ConcurrentHopscotchLocal : public GenericConcurrentHopscotchLocal {
private:
  ...
  ConcurrentHopscotchLocal(uint8_t ds_id, uint32_t local_num_entries_shift,
                      uint32_t remote_num_entries_shift,
                      uint64_t remote_data_size);
  NOT_COPYABLE(ConcurrentHopscotchLocal);
  NOT_MOVEABLE(ConcurrentHopscotchLocal);
...
#include "internal/concurrent_hopscotch_local.ipp"
